...
Subatom validates request body, query, params, headers, and files through its route schema system. Subatom Infer is the native ecosystem validator, and the validation layer can consume schemas that expose the supported Standard Schema protocol.

## Overview

Validation in Subatom:
- Supports schemas implementing the supported Standard Schema protocol
...
## Supported Schema Protocol

`subatom-infer` is the native validation package in the Subatom ecosystem. For other validators, verify that the version you use exposes the Standard Schema interface expected by the installed Subatom version.
