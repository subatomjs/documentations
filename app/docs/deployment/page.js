...
export const metadata = {
  ...
  description: "Production operations guide: Nginx reverse proxy configuration, TLS termination, ALB timeouts, and signal handling.",
};
  ...
  return (
    <div className="flex">
      <div className="flex-1 min-w-0">
        <Breadcrumbs items={[{ label: "Production Deployment", href: "/docs/deployment" }]} />
        ...
        <p className="text-surface-600 dark:text-slate-300 text-sm leading-relaxed mb-6">
          Always terminate TLS at your reverse proxy, disable proxy buffering, and set read timeouts higher than <code className="font-mono">heartbeatIntervalMs + heartbeatTimeoutMs</code> (minimum 75s).
        </p>

        <section id="nginx" className="mb-10 scroll-mt-20">
          <h2 className="text-xl font-bold text-surface-900 dark:text-white mb-2">Nginx Reverse Proxy</h2>
          <CodeBlock
        ...
        <section id="caddy" className="mb-10 scroll-mt-20">
          <h2 className="text-xl font-bold text-surface-900 dark:text-white mb-2">Caddy v2 Configuration</h2>
          <CodeBlock
            filename="Caddyfile"
        ...
        <section id="alb" className="mb-10 scroll-mt-20">
          <h2 className="text-xl font-bold text-surface-900 dark:text-white mb-2">AWS Application Load Balancer (ALB)</h2>
          <div className="p-4 rounded-xl border border-surface-200 dark:border-surface-800 bg-surface-50 dark:bg-surface-900 text-xs text-surface-600 dark:text-slate-300 space-y-2">
            <p>1. Forward HTTPS port 443 with ACM certificate to HTTP target group on port 3000.</p>
            <p>2. Set target group health check path to <code className="font-mono">/health</code>, status 200.</p>
            <p>3. Set ALB idle timeout attribute to at least <strong>75 seconds</strong>.</p>
          </div>
        </section>

        <Pagination
          prev={{ title: "Metrics & Telemetry", href: "/docs/api/observability" }}
        />
      </div>
